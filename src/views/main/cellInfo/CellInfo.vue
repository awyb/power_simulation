  ...
  <div class="cell-info">
    <div class="top">
      <div class="description">
        ...
        <el-input v-model="description.namec" class="name-input" style="width: 300px;"/>
      </div>
    ...
    <el-tabs v-model="actTab" class="info-tabs" @tab-click="handleClick">
      <el-tab-pane label="参数" name="param" v-if="graphType==='node'||graphType==='edge'" style="height: 100%;">
        <el-collapse v-model="collapseExp.main" v-if="graphType==='node'">
          <el-collapse-item name="attr" title="属性">
            <att-form v-if="attConfig" :attFlds="attConfig" style="width: 300px;padding: 10px 20px 10px 40px;"></att-form>
          </el-collapse-item>
          <el-collapse-item name="param" title="参数">
            <el-collapse v-model="collapseExp.param" style="padding-left: 20px;">
              <el-collapse-item v-for="collapse in collapseItem" :key="collapse.classify" :name="collapse.classify" :title="collapse.classify" v-show="collapse.show">
                <template #title>
                  <el-tooltip :content="collapse.classifydescribe" placement="left" effect="light">
                    <span>{{ collapse.classify }}</span>
                  </el-tooltip>
                </template>
                <att-form :attFlds="collapse.children"  style="width: 300px;padding: 10px 20px;" @hidden-classify="hiddenClassify"></att-form>
              </el-collapse-item>
            </el-collapse>
          </el-collapse-item>
      ...
      <el-tab-pane label="图纸选项" name="graphConfig" v-if="graphType==='blank'">
        <dyn-form :params="csparams" @onChange="updState"></dyn-form>
      </el-tab-pane>
    </el-tabs>
  </div>
</template>
...
import { variable, nodeParams, attFld, collapseItem } from '@/components/interface/interfaceBase'
...
export default defineComponent({
  ...
  setup(props, { expose })
  {
    ...
    const description = ref({namec:'图纸', classify:'电力仿真-画布', icon:'icon-huabu'})
    ...
    watch(() => props.params, async(newValue) =>
    {
      ...
      {
        ports.value = newValue.cell.data.ports.map((p:any)=>({...p, isval:true, unit:''}))
        const config = await common.getNodeConfig(newValue.cell.data.name)
        config.forEach(c =>
        {
          c.show = true
          ...
          if (c.children)
          {
            c.children.forEach(cc =>
            {
              ...
              cc.value = cc.defval
              cc.value_ = cc.defval
              if (newValue.cell.data.params && newValue.cell.data.params[cc.classify])
              {
                cc.value = newValue.cell.data.params[cc.classify][cc.name]
                cc.value_ = newValue.cell.data.params[cc.classify][cc.name]
              }
              if (cc.disptype === 2)
              {
    ...
    (function()
    {
      ...
      common.getAttr().then(obj =>
      {
        ...
        {
          const t:attFld = { ...fld, isval: 1, value: fld.defval, value_: fld.defval, disabled: false}
          if (fld.disptype === 2)
          {
