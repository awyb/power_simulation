...
<template>
  <el-container class="container">
      ...
      <div style="width: 100%;height: 100%;background: #f3f3f3;">
        <el-collapse v-model="expArr">
          ...
          <el-collapse-item name="2" title="demo2">
            <div v-for="item in moduleList.slice(2)"
              :key="item.id"
              draggable="true"
              @dragend="handleDragEnd($event, item)"
              @contextmenu="handleRightClick($event, item)"
    ...
    <el-aside :width="collapseR?'300px':'0'" style="padding: 0;">
      <div style="width: 100%;height: 100%;background: #f3f3f3;">
        {{ nodeInfo }}
      </div>
    </el-aside>
    ...
    <el-button class="collapse-btn" size="small" :style="{right:collapseR?'300px':'0px'}" @click="collapseR=!collapseR" :icon="!collapseR?DArrowLeft:DArrowRight"></el-button>
  </el-container>
</template>
